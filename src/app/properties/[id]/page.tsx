...
export default function PropertyDetailPage() {
  ...
  // ✅ البحث عن العقار مع التحقق
  const property = state.properties.find(p => p.id === id);
  
  // ✅ حالات الاستفسار
  const [showInquiryModal, setShowInquiryModal] = useState(false);
  const [inquiryType, setInquiryType] = useState<InquiryType>('شراء');
  ...
  // ✅ حالة عدم وجود العقار
  if (!property) {
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 text-center">
        <h2 className="text-3xl font-bold text-white">العقار غير موجود</h2>
        <p className="text-gray-400 mt-2">العقار المطلوب غير موجود或 تم حذفه</p>
        <Link 
          href="/properties"
  ...
  // ✅ فتح واتساب صاحب العقار
  const openOwnerWhatsapp = () => {
    const number = property.ownerWhatsapp || property.ownerPhone;
    if (number) {
      window.open(
        `https://wa.me/212${number.replace(/^0/, '')}?text=مرحباً، أنا مهتم بالعقار #${property.id}`, 
        '_blank'
      );
    }
  };
          ...
          <div className="rounded-2xl overflow-hidden mb-4 relative group">
            {hasImages ? (
              ...
              <div className="w-full h-[400px] bg-slate-700 flex items-center justify-center">
                <span className="text-gray-400 text-xl">لا توجد صور</span>
              </div>
            )}
              ...
              <div className="flex flex-wrap gap-3">
                {property.ownerPhone && (
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-medium transition-all duration-200 hover:scale-105 shadow-lg"
                  >
                ...
                {(property.ownerWhatsapp || property.ownerPhone) && (
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-3 bg-green-600 hover:bg-green-700 rounded-lg font-medium transition-all duration-200 hover:scale-105 shadow-lg"
                  >
                ...
                {!property.ownerWhatsapp && !property.ownerPhone && (
                  <p className="text-amber-300/70 text-sm">⚠️ لم يتم إضافة رقم اتصال لهذا العقار</p>
                )}
              </div>
            </div>
          )}

          {/* ===== قسم الزوار والموظفين - نموذج الاستفسار ===== */}
          {!isAdmin && (
            <div className="mb-8">
              <div className="bg-gradient-to-r from-emerald-900/40 to-teal-900/40 p-6 rounded-xl border border-emerald-700/50">
                ...
                <p className="text-gray-400 text-sm mb-4">تواصل معنا وسنرد عليك في أقرب وقت!</p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <button
                      onClick={() => { setShowInquiryModal(true); setInquiryType('زيارة'); }}
                      className="flex flex-col items-center gap-2 p-4 bg-blue-600/20 hover:bg-blue-700 rounded-xl font-medium transition-all duration-200 hover:scale-[1.05]"
                    >
                    ...
                    <button
                      onClick={() => { setShowInquiryModal(true); setInquiryType('شراء'); }}
                      className="flex flex-col items-center gap-2 p-4 bg-emerald-600/20 hover:bg-emerald-700 rounded-xl font-medium transition-all duration-200 hover:scale-[1.05]"
                    >
                      <span className="text-2xl">🛒</span>
                    ...
                    <button
                      onClick={() => { setShowInquiryModal(true); setInquiryType('كراء'); }}
                      className="flex flex-col items-center gap-2 p-4 bg-purple-600/20 hover:bg-purple-700 rounded-xl font-medium transition-all duration-200 hover:scale-[1.05]"
                    >
                      <span className="text-2xl">🔑</span>
