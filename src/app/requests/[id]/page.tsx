        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 shadow-xl">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-900/50 rounded-xl p-4">
                  ...
                  <p className="text-lg font-semibold text-white">{new Date(request.createdAt).toLocaleDateString('ar-DZ') || 'غير محدد'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 shadow-xl">
              ...
              {request.area > 0 && (<div className="flex items-center justify-between bg-slate-900/50 rounded-xl p-4 mt-3"><span className="text-slate-400">📐 المساحة</span><span className="text-lg font-semibold text-blue-400">{request.area} م²</span></div>)}
            </div>

            {request.notes && (<div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 shadow-xl"><h2 className="text-xl font-bold text-white mb-4">📝 ملاحظات</h2><div className="bg-slate-900/50 rounded-xl p-4"><p className="text-slate-300 whitespace-pre-wrap">{request.notes}</p></div></div>)}
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 shadow-xl sticky top-6">
              ...
              <div className="space-y-3">
                <Link href={`/dashboard/clients?id=${request.clientId}`} className="block w-full bg-emerald-500 hover:bg-emerald-600 text-white text-center py-3 px-4 rounded-xl font-bold transition-all shadow-lg">👤 عرض بيانات العميل</Link>
                <button className="w-full bg-blue-500 hover:bg-blue-600 text-white py-3 px-4 rounded-xl font-bold transition-all shadow-lg">💬 تواصل</button>
                <button className="w-full bg-red-500 hover:bg-red-600 text-white py-3 px-4 rounded-xl font-bold transition-all shadow-lg">🗑️ حذف الطلب</button>
              </div>
            </div>
          </div>
        </div>
